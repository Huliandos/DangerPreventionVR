...
public class VR_HumanoidAvatarAnimationController_SeperatedHands : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Transform Reference to the avatars hands (seperated Rig). In this order: L, R")]
    Transform[] hands;

    [SerializeField]
    [Tooltip("Transform Reference to the avatars wrists. In this order: L, R")]
    Transform[] wrists;
    ...
    void Start()
    {
        ...
        */
        ...
        neckOffset = neck.position - eyes.position;
        ...
        armsStartRots[0] = wrists[0].parent.parent.localRotation;
        armsStartRots[1] = wrists[1].parent.parent.localRotation;
    ...
    void Update()
    {
        ...
        {
            Transform upperArm = wrists[i].parent.parent;    //very hardcoded TODO: change later
            ...
            wrists[i].GetComponent<InverseKinematics_AdjustableRootrot>().RotatedRootBone();
        }
    ...
    void adjustArmsPosition()
    {
        ...
        {
            //handTargets[i].position = hands[i].position + handsOffset[i];
            //handTargets[i].rotation = hands[i].rotation;
            handTargets[i].position = hands[i].position + hands[i].TransformDirection(handsOffset[i]);
            handTargets[i].rotation = Quaternion.LookRotation(hands[i].forward, hands[i].position - handTargets[i].position);

            //ToDo: will prolly have to add smth like this
            //hands[i].parent.position = hands[i].position + handsOffset[i];
            //hands[i].parent.rotation = hands[i].rotation;
            hands[i].parent.position = hands[i].position + hands[i].TransformDirection(handsOffset[i]);
            //hands[i].parent.rotation = Quaternion.LookRotation(hands[i].forward, hands[i].position - hands[i].parent.position);
        }
    }
