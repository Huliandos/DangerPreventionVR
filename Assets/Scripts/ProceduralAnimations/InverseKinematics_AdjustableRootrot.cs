...
public class InverseKinematics_AdjustableRootrot : MonoBehaviour
{
    [SerializeField]
    ...
    #region rotStorage
    Quaternion[] localBaseRots;
    #endregion

    // Start is called before the first frame update
    void Awake()
    {
        localBaseRots = new Quaternion[chainLength];
        Transform currentTrans = transform;

        for (int i = 0; i < chainLength; i++)
        {
            localBaseRots[i] = currentTrans.localRotation;

            currentTrans = transform.parent;
        }
    ...
    public void init() {
        ...
        //localRotation
        //startRotationTarget = target.localRotation;
    ...
    void resolveIK() {
        ...
        //localRotation
        //Quaternion rootRot = (bones[0].parent != null) ? bones[0].parent.localRotation : Quaternion.identity;
        //Quaternion rootRotDiff = rootRot * Quaternion.Inverse(startRotationRoot);
        ...
        for (int i = 0; i < bones.Length; i++)
        {
            ...
            if (i == positions.Length - 1)
            {
                ...
                //localRotation
                //bones[i].localRotation = target.localRotation * Quaternion.Inverse(startRotationTarget) * startRotationBone[i];
            ...
            else
            {
                ...
                //localRotation
                //bones[i].localRotation = Quaternion.FromToRotation(startDirectionToSuccesor[i], positions[i + 1] - positions[i]) * startRotationBone[i];
            }
        }
    }

    public void RotatedRootBone()
    {
        Transform currentTrans = transform;

        for (int i = 0; i < chainLength; i++)
        {
            currentTrans.localRotation = localBaseRots[i];
