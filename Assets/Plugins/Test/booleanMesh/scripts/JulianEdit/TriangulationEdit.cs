...
public class TriangulationEdit {
	...
	public TriangulationEdit(Mesh mesh){

		int i;
		this.lowerAngle = 1f;
		this.vertices = new List<Vertex>();
		for(i=0;i< mesh.vertices.Length;i++) 
			this.vertices.Add(new Vertex(mesh.vertices[i],0, mesh.normals[i], mesh.uv[i]));
		this.triangles = new List<Polygon>();
		for(i=0;i< mesh.triangles.Length;i+=3) this.triangles.Add(new Polygon(new int[3]{ mesh.triangles[i] , mesh.triangles[i+1] , mesh.triangles[i+2]}));

	}

	public TriangulationEdit Clone(){

		TriangulationEdit triangulation = new TriangulationEdit();
		int i;
	...
	bool existOnTriangle (Vector3 worldPosition, int onTriangle){
		Debug.Log("Exist on Triangle: " + triangles.Count +  " " + onTriangle);
