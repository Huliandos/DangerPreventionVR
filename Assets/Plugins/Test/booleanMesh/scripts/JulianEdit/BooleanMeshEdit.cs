...
public class BooleanMeshEdit {

	GameObject ObjectA;
	GameObject ObjectB;

	TriangulationEdit triangulationA;
	TriangulationEdit triangulationB;

	float distance, customDistance;

	public BooleanMeshEdit(GameObject A, GameObject B){
		
		this.ObjectA = A;
		this.ObjectB = B;
		this.triangulationA = new TriangulationEdit(A.GetComponent<MeshFilter>().mesh);
		this.triangulationB = new TriangulationEdit(B.GetComponent<MeshFilter>().mesh);
		this.distance = 100f;
	...
	class intersectionDATA {
		...
		public intersectionDATA(TriangulationEdit a, TriangulationEdit b, Collider m){
	...
	void intersectionRay(int originVertice, int toVertice, intersectionDATA var){
		...
		if(var.A.vertices[var.A.triangles[var.triangleA].indexVertice[originVertice]].type == 0) 
			if(var.colliderB.Raycast(var.r1, out var.hit, var.customDistance)) 
				intersectionPoint(var);

		if(var.A.vertices[var.A.triangles[var.triangleA].indexVertice[toVertice]].type == 0) 
			if(var.colliderB.Raycast(var.r2, out var.hit, var.customDistance)) 
				intersectionPoint(var);
	...
	void AInToB(intersectionDATA var){
		...
		for(int i=0;i<var.A.vertices.Count;i++){
			...
			if(In(var.colliderB, var.A.vertices[i].pos)) var.A.vertices[i].type = -1; //In
	...
	void clearVertices(TriangulationEdit triangulation, int t){
	...
	void intersections(){
		...
		//IntersectionDATA
		intersectionDATA varA = new intersectionDATA(this.triangulationA,this.triangulationB,this.ObjectB.GetComponent<Collider>());
		intersectionDATA varB = new intersectionDATA(this.triangulationB,this.triangulationA,this.ObjectA.GetComponent<Collider>());
	...
	bool ConcaveHull(Collider colliderB, Vector3 position, Ray ray, RaycastHit hit){
		...
		while(colliderB.Raycast(tempRay, out tempHit, customDistance)){
			
			if(tempHit.triangleIndex == hit.triangleIndex) break;
			ray.origin = -ray.direction * customDistance + position;
			
			if(!colliderB.Raycast(ray, out hit, customDistance)) return true;
	...
	bool In(Collider colliderB, Vector3 position) {
		...
		r = colliderB.Raycast(right   , out rightHit   , distance);
		l = colliderB.Raycast(left    , out leftHit    , distance);
		u = colliderB.Raycast(up      , out upHit      , distance);
		d = colliderB.Raycast(down    , out downHit    , distance);
		f = colliderB.Raycast(forward , out forwardHit , distance);
		b = colliderB.Raycast(back    , out backHit    , distance);
		
		if(r&&l&&u&&d&&f&&b) {
			
			if(!ConcaveHull(colliderB, position,right,rightHit))
				if(!ConcaveHull(colliderB, position,left,leftHit))
					if(!ConcaveHull(colliderB, position,up,upHit))
						if(!ConcaveHull(colliderB, position,down,downHit))
							if(!ConcaveHull(colliderB, position,forward,forwardHit))
								if(!ConcaveHull(colliderB, position,back,backHit)) return true;
